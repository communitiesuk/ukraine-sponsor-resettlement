...
const show_hide = () => {
  ...
  cy.get('body').then(($body) => {
    if ($body.find(elements.show_true).length > 0) {
      //evaluates as show if button exists at all
      cy.get(elements.show_true).should('be.visible')
      cy.log('aria expanded = Yes')
    } else if ($body.find(elements.show_false).length > 0) {
      //evaluates as show if button exists at all
...
const cannot_uts_heading = () => {
  cy.get(elements.main_heading).contains('You cannot use this service').should('be.visible')
}
...
const why_need = () => {
  cy.get(elements.summary_text).click().contains('Why do I need this?').should('be.visible')
  cy.get(elements.details_text).click().contains('To sponsor someone under the Homes for Ukraine Scheme, you must be a British or Irish Citizen or have settled status in the UK (which means having the right to live in the UK permanently).').should('be.visible')
}
...
export const uam_step_10_l2 = () => {
  ...
  email()
  cy.xpath(elements.save_return).click().wait(Cypress.env('waitTime')) //save and return will clear the email text fields
  email()
  Continue()
  mobile()
  cy.xpath(elements.save_return).click().wait(Cypress.env('waitTime')) //save and return will clear the email text fields
...
const i_am_not_sure_tname_link = () => {
  ...
  cy.get(elements.details_text)
    .click()
    .contains('Enter their name as shown on their identity documents.')
    .should('be.visible')
